...
def run_experiment(params):
    ...
    for first_episode in [0, params.num_episodes]:
        env = memory_architecture(RecordStore)(
            # record store
        ...
        episodes = range(
            first_episode,
            first_episode + params.num_episodes,
            params.eval_frequency,
        )
        results_dir = Path(DIRECTORY, 'results', 'transfer')
...
PSPACE = PermutationSpace(
    ['random_seed', 'max_internal_actions', 'num_albums'],
    random_seed=[
...
PSPACE.add_filter(
    lambda num_albums, max_internal_actions:
        num_albums == 1000 or max_internal_actions == 1
)
