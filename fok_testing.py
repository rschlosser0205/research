...
def test_networkxkb():
    ...
    store.store(0, True, 'node_cat', is_a='node_mammal', has='node_fur', name='cat')
    print('cat ' + str(store.get_activation('node_cat', 0)))
    store.store(1, True, 'node_bear', is_a='node_mammal', has='node_fur', name='bear')
    print('cat ' + str(store.get_activation('node_cat', 1)))

    store.store(2, True, 'node_whale', is_a='node_mammal', lives_in='node_water')
    store.store(3, True, 'node_whale', name='whale')
    #print(store.get_activation('node_whale', 3))
    store.store(4, True, 'node_fish', is_a='node_animal', lives_in='node_water')
    store.store(5, True, 'node_mammal', has='node_vertebra', is_a='node_animal')
    # retrieval
    ...
    # query traversal
    store.store(9, True, 'node_cat')
    # at this point, whale has been activated thrice (from the store and the retrieve)
    print('bear ' + str(store.get_activation('node_bear', 10)))
    print('cat ' + str(store.get_activation('node_cat', 10)))
    print(store.graph.nodes['node_whale']['activation'])
    # while cat has been activated twice (from the store)
    ...
    print('bear ' + str(store.get_activation('node_bear', 12)))
